...
@section('content')
    <div class="container">
        ...
        <div class="mt-4 shadow-lg mb-5 bg-body rounded">
            <form class="row g-3 p-5 forma" method="post" action="{{ route('tasks.store') }}">
                @csrf
                <div class="col-md-12">
                    <label for="title" class="form-label">Название задачи</label>
                    <input type="text" value="{{ old('title') }}" placeholder="Убрать двор" name="title"
                        class="form-control" id="title">
                </div>
                <div class="col-md-12">
                    <label class="form-label">Комментарий (при необходимости)</label>
                    <textarea name="description" placeholder="желательно хорошо убрать" class="form-control" cols="20" rows="5">{{ old('description') }}</textarea>
                </div>
